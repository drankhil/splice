...
export const expirationSchema = z.string().refine(date => dayjs(date).isValid(), {
  message: 'Invalid Expiry Date',
});

export const effectiveDateSchema = z.string().refine(date => dayjs(date).isValid(), {
...
export const svWeightSchema = z
  .string()
  .min(1, { message: 'Weight is required' }) // Rejects ""
