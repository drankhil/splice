...
function configureGateway(
  ...
  const servers = Array.from({ length: numMigrations }, (_, i) => i).flatMap(migration =>
    [0, 1, 2, 3, 4].flatMap(node => ({
      ...
      port: {
        ...
        number: Number(`26${migration}${node}6`),
        protocol: 'TCP',
      },
    }))
  );
  ...
  console.error(
    `highest migratin ID: ${DecentralizedSynchronizerUpgradeConfig.highestMigrationId}`
  );
...
export function configureIstio(
  ...
  return configureGateway(ingressNs, gwSvc, cometBftSvc, publicGwSvc);
}
